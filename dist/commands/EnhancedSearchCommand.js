...
exports.EnhancedSearchCommand = void 0;
const discord_js_1 = require("discord.js");
const BaseCommand_1 = require("./BaseCommand");
class EnhancedSearchCommand extends BaseCommand_1.BaseCommand {
    constructor(config) {
        super('розширений_пошук', '🔍 Покращений пошук з діапазонами та сортуванням', config, (builder) => {
            return builder
                ...
                .setRequired(false)
                .addChoices({ name: 'Назва', value: 'назва' }, { name: 'Ціна', value: 'ціна' }, { name: 'Кількість', value: 'кількість' }, { name: 'Контрагент', value: 'контрагент' }, { name: 'Серія', value: 'серія' }))
                .addStringOption((option) => option
                .setName('порядок')
                .setDescription('Порядок сортування')
                .setRequired(false)
                .addChoices({ name: 'За зростанням', value: 'asc' }, { name: 'За спаданням', value: 'desc' }));
        });
    }
    /**
     * Виконання команди
    ...
    getActiveFilters(filters) {
        ...
        if (filters.priceFrom || filters.priceTo) {
            ...
            if (filters.priceFrom)
                priceRange.push(`від ${filters.priceFrom}`);
            if (filters.priceTo)
                priceRange.push(`до ${filters.priceTo}`);
            activeFilters.push(`💰 Ціна: ${priceRange.join(' ')} грн`);
        }
        if (filters.quantityFrom || filters.quantityTo) {
            const quantityRange = [];
            if (filters.quantityFrom)
                quantityRange.push(`від ${filters.quantityFrom}`);
            if (filters.quantityTo)
                quantityRange.push(`до ${filters.quantityTo}`);
...
exports.EnhancedSearchCommand = EnhancedSearchCommand;
//# sourceMappingURL=EnhancedSearchCommand.js.map