...
export class AIService extends BaseServiceClass {
   ...
   */
  public async analyzeDocumentStructure(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
    ...
    return await this.generateResponse(prompt, { useCache: true });
  }

  /**
   * 📝 Підсумування вмісту документа
   */
  public async summarizeDocumentContent(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
  ...
  /**
   * ✅ Витяг дій з документа
   */
  public async extractActionItems(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
  ...
  /**
   * ❓ Генерація питань та відповідей
   */
  public async generateQnA(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
  ...
  /**
   * ⚖️ Перевірка відповідності стандартам
   */
  public async checkCompliance(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
   ...
   * 🌐 Переклад документа
   */
  public async translateDocument(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
  ...
  /**
   * 🔍 Аналіз якості документа
   */
  public async assessDocumentQuality(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
  ...
  /**
   * 📊 Аналіз даних документа
   */
  public async analyzeDocumentData(content: string): Promise<AIResponse> {
    const prompt = `
Виконай аналіз даних у цьому документі:

1. Витягни усі числові дані та статистику
2. Визнач тренди та закономірності
3. Вияви аномалії або відхилення
4. Запропонуй візуалізації для кращого розуміння

Надай аналітичний звіт з висновками.
...
`;
  ...
  /**
   * 🔐 Огляд безпеки документа
   */
  public async reviewDocumentSecurity(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
  ...
  /**
   * 🔄 Аналіз змін між версіями
   */
  public async analyzeVersionChanges(oldContent: string, newContent: string): Promise<AIResponse> {
    const prompt = `
...
`;
  ...
  /**
   * 📈 Прогнозування ефективності документа
   */
  public async predictDocumentPerformance(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
   ...
   */
  public async analyzeStakeholders(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
  ...
  /**
   * 💰 Аналіз бюджету в документі
   */
  public async analyzeBudget(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
   ...
   * ⚠️ Оцінка ризиків
   */
  public async assessRisks(content: string): Promise<AIResponse> {
    const prompt = `
...
`;
   ...
   */
  public async segmentAudience(content: string): Promise<AIResponse> {
    const prompt = `
